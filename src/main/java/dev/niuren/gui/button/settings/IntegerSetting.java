...
public class IntegerSetting extends SettingButton {
    ...
    public IntegerSetting(Module module, Setting setting, int x, int y, int w, int h) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
        ...
        BetaX.GUI.drawFlat(context, x() + 3, y() + Modules.get().get(ClickGUI.class).sliderHeight.get(), (int) (x() - 2 + (sliderWidth) + 5), y() + h(), hover ? HUD.INSTANCE.colorType().darker().getRGB() : HUD.INSTANCE.colorType().getRGB());
        ...
        FontRenderers.Arial.drawString(context.getMatrices(), String.valueOf(setting.get()), (x() + 6) + mc.textRenderer.getWidth(" " + setting.getName()), y() + 4, WHITE);
        // context.drawTextWithShadow(mc.textRenderer, String.valueOf(setting.get()), (x() + 6) + mc.textRenderer.getWidth(" " + setting.getName()), y() + 4, WHITE);
    }
    ...
    public void mouseDown(int mouseX, int mouseY, int button) {
        if (!Modules.get().isActive(ClickGUI.class) || button == 1) {
            dragging = false;
            return;
        }
    ...
    public static class Slider extends IntegerSetting {
        ...
        protected void updateSlider(int mouseX) {
            ...
            if (dragging) {
                setting.setValue(diff == 0.0 ? setting.getMin() : (int) Double.parseDouble(String.format("%." + setting.getInc() + "f", diff / w() * (max - min) + min).replace(",", ".")));
                BetaX.EVENT_BUS.post(new StateEvent.Number(this.module(), setting));
            }
        }
    }
}

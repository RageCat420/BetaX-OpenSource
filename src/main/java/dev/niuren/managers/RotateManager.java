package dev.niuren.managers;


import dev.niuren.BetaX;
import dev.niuren.events.Cancelled;
import dev.niuren.events.event.PacketEvent;
import dev.niuren.events.event.RotateEvent;
import dev.niuren.events.event.UpdateWalkingEvent;
import dev.niuren.ic.Wrapper;
...
public class RotateManager implements Wrapper {
    ...
    public void onRotation(RotateEvent event) {
        ...
        if (directionVec != null && !ROTATE_TIMER.passed((long) (CombatSetting.INSTANCE.rotateTime.get() * 1000))) {
            float[] angle = EntitiesUtil.getLegitRotations(directionVec);
    ...
    public void onPacketSend(PacketEvent.Send event) {
        ...
        if (!EntitiesUtil.rotating && CombatSetting.INSTANCE.rotateSync.get()) {
    ...
    public void onUpdateWalkingPost(UpdateWalkingEvent event) {
        if (event.getStage() == Cancelled.Stage.Post)
            setRotation(lastYaw, lastPitch, false);
    }
    ...
    public void onUpdateWalking(UpdateWalkingEvent event) {
        ...
        if (event.getStage() == Event.Stage.Pre) {
            ...
            flybird.EVENT_BUS.post(rotateEvent);
